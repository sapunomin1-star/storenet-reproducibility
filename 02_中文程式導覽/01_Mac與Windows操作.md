...
**重跑的曲線可能與報告不同。** 同一筆最低電費可能對應多種充放電時程，MATLAB 版本不同時可能選到不同的一組。例如本次 R2026a 測試的 VPP-BM 電費與報告一致，尖峰卻為 67.75 kW，報告保存的結果為 75.50 kW；VPP-BM 本身沒有要求降低尖峰。因此測試會核對電費、各策略的最佳化目標和電池限制，不要求所有曲線逐點相同。若要看報告使用的原圖，請直接開啟 PDF 或 [保存的代表日結果](../StoreNet/results/b2022_ir_v1_formal/b2022_typical_v1_20200824/)。
